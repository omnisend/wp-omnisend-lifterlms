...
define( 'OMNISEND_LIFTERLMS_ADDON_VERSION', '1.1.0' );
...
add_action( 'admin_init', array( 'Omnisend_LifterLMSAddOn', 'add_privacy_policy_content' ) );
...
class Omnisend_LifterLMSAddOn {
	...
	public static function add_privacy_policy_content(): void {
		...
		$content =
			'<p>' . esc_html__( 'The Omnisend for LifterLMS Add-On syncs your LifterLMS student and registration data to Omnisend for email and SMS marketing purposes. Depending on your setup, this may include your email address, first and last name, phone number, the courses and memberships you are enrolled in, your consent choices, and other profile information.', 'omnisend-lifter_lms' ) . '</p>' .
			'<p>' . esc_html__( 'This data is transmitted to and stored by Omnisend, a third-party service, and is retained there according to Omnisend’s data retention practices for as long as your contact record exists. Existing students may also be synced to Omnisend in bulk when the add-on is activated.', 'omnisend-lifter_lms' ) . '</p>' .
			'<p>' . esc_html__( 'When the accompanying Omnisend plugin is active, a tracking snippet may also set cookies in visitors’ browsers to identify contacts and track their activity on the site.', 'omnisend-lifter_lms' ) . '</p>' .
			'<p>' . sprintf(
				/* translators: 1: Omnisend Privacy Policy URL, 2: Omnisend Terms of Use URL */
				esc_html__( 'You have the right to request access to, export of, or deletion of your personal data. For details on how Omnisend processes personal data and how to exercise these rights, see Omnisend’s Privacy Policy at %1$s and Terms of Use at %2$s.', 'omnisend-lifter_lms' ),
		...
		wp_add_privacy_policy_content( OMNISEND_LIFTERLMS_ADDON_NAME, wp_kses_post( $content ) );
	}
	...
	public static function check_plugin_requirements(): void {
		...
		$lifterlms_addon_plugin = 'omnisend-for-lifterlms/class-omnisend-lifterlmsaddon.php';
	...
	public static function omnisend_is_not_connected_notice() {
		echo '<div class="error"><p>' . esc_html__( 'Your Omnisend is not configured properly. Please configure it by connecting to your Omnisend account.', 'omnisend-lifter_lms' ) . '<a href="https://wordpress.org/plugins/omnisend/">' . esc_html__( 'Omnisend plugin.', 'omnisend-lifter_lms' ) . '</a></p></div>';
	}
	...
	public static function omnisend_is_not_installed_notice() {
		echo '<div class="error"><p>' . esc_html__( 'Omnisend plugin is not installed. Please install it and connect to your Omnisend account.', 'omnisend-lifter_lms' ) . '<a href="https://wordpress.org/plugins/omnisend/">' . esc_html__( 'Omnisend plugin.', 'omnisend-lifter_lms' ) . '</a></p></div>';
	}
	...
	public static function lifterlms_notice(): void {
		echo '<div class="error"><p>' . esc_html__( 'Plugin Omnisend for LifterLMS Add-On is deactivated. Please install and activate LifterLMS plugin.', 'omnisend-lifter_lms' ) . '</p></div>';
	}
}
